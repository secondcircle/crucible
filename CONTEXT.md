...
**Rule**:
One TypeScript file in `.crucible/rules/`, with a companion test file, that
says how the repo wants one thing done: a trigger, a scope, a mode (off,
shadow, enforce), an extractor that pulls out the narrow piece it governs, a
judge's questions, a decision and the feedback an agent reads. Versioned with
the repo it governs; Crucible runs it, the repo owns it.
_Avoid_: policy, check, lint rule, guard.

**Judge**:
The classifier a rule's questions go to, over the state its extractor built:
Jev today, a model on the user's own provider where Jev is not allowed. A
deterministic rule has none.
_Avoid_: classifier (alone), evaluator, oracle.

**Firing**:
One evaluation of a rule that produced something: an item judged or decided
(pass, note, escalate, block, hold), or a skip that means something is wrong
(extractor threw or ran over budget, judge unreachable). An admitted event
that extracted nothing is not a firing; it is counted, never listed.
_Avoid_: hit, trigger, evaluation, event (the input, not the record).

**Outcome**:
What came of a firing, read from the item's content key at later edits and
at turn end or node completion: fixed (the item is gone), reworded (a new
item at the same place, judged again), open (not yet seen again), ignored
(still there when the turn ended or the node completed).
_Avoid_: result, resolution, compliance, verdict (that is the judge's).

**Rules board**:
The workspace-scoped overlay behind the § chip in the top bar: rules that
need attention first, then every rule with how often it was reached, what it
decided, what came of it, how long it took and what it cost, then the chosen
rule's firings, with one firing read whole beside them. Scoped to the whole
workspace, one session or one run; windowed to today, 7 days, 30 days or
all. It reports; it never toggles a rule's mode and never answers anything.
_Avoid_: rules dashboard, health monitor, rules view (the sketch's word),
rules panel.

**Rule mark**:
The § tag on the tool call a rule judged, in a session transcript or a run
node transcript, naming the rule and what it did; a shadow rule's reads
"would note". Click opens that firing on the rules board. A note delivered
inline shows inside the tool result under the mark; a steered note is its
own message between tool chains.
_Avoid_: rule badge, annotation, inline verdict.
