...
//
// The rules are pure, so the two that are easy to get wrong are provable
// without a document: what "not looking" means, and what order Tab walks.
...
describe('what counts as unwatched', () => {
  it('marks a session nobody was looking at', () => {
    expect(finishedUnwatched('s2', { activeSessionId: 's1', windowFocused: true })).toBe(true)
  })

  it('leaves the session on screen alone while the window has focus', () => {
    expect(finishedUnwatched('s1', { activeSessionId: 's1', windowFocused: true })).toBe(false)
  })

  it('marks even the session on screen when Crucible is behind another app', () => {
    expect(finishedUnwatched('s1', { activeSessionId: 's1', windowFocused: false })).toBe(true)
  })
})
