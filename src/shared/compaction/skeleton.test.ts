...
import { ANSWER_BATCH_PREFIX, composeAnswerBatch } from '../questions/wording'
...
describe('a skeleton an earlier build stored', () => {
  ...
  // An answer batch is Crucible's envelope around the person's own rulings:
  // re-read, the envelope goes and what they typed stays as their words.
  it('re-reads a stored answer batch as what the person typed', () => {
    expect(reclassifySkeleton(stored)[2]).toEqual({
      kind: 'user',
      text: 'Answered: (1) Merge? → “yes”'
    })
  })
