...
beforeEach(() => {
  root = join(
    mkdtempSync(join(tmpdir(), "crucible-stager-")),
    "update-staging",
  );
  ran = [];
});
...
describe("staging a version", () => {
  ...
  // The last staged tree always holds electron's `default_app.asar`. Under
  // Electron's patched `fs` that file stats as a directory and the recursive
  // rm leaves it behind with ENOTEMPTY, every time, so the app never updates
  // again. The clear turns the shim off (`process.noAsar`) and puts it back.
  // Plain Node has no shim, so this pins the flag's handling and the tree's
  // removal; the shim itself is Electron's to test.
  it("clears a tree holding an asar archive and leaves process.noAsar as it found it", async () => {
    ...
    writeFileSync(join(resources, "default_app.asar"), "not really an archive");
    const before = process.noAsar;
    let duringClear: boolean | undefined;
    const watchful = async (): Promise<void> => {
      duringClear = process.noAsar;
    };
    ...
    expect(duringClear).toBe(before);
    expect(process.noAsar).toBe(before);
  });
});
