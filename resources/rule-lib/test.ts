...
import type { Action, RuleEvent } from './rule.ts'
import { commentBlocks } from './extract.ts'
import { show } from './git.ts'

export type EventSource = (src: ev.Source) => readonly RuleEvent[] | Promise<readonly RuleEvent[]>
...
// On the global object rather than in this module: the runner and a rule's
// test file can each be handed their own copy of this module, and both must
// see one list.
const REGISTRY = Symbol.for('crucible.rule.scenarios')

export function registry(): Scenario[] {
  const holder = globalThis as { [REGISTRY]?: Scenario[] }
  holder[REGISTRY] ??= []
...
/** Whether a scenario can only be answered by calling the judge. */
export function needsJudge(expect: Expect): boolean {
  return expect.actions !== undefined || expect.feedback !== undefined || expect.feedbackIncludes !== undefined
}
...
export const bash =
  (command: string): EventSource =>
  (src) => [ev.bash(src, command)]

export const checkpoint =
  (base: string, head: string): EventSource =>
  (src) => [ev.checkpoint(src, base, head)]

/** A real commit as a commit rule sees it. */
export const commitEvent =
  (sha: string): EventSource =>
  (src) => [ev.commitEvent(src, sha)]
