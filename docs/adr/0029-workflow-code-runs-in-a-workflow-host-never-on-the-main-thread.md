...
suite, five minutes at a time, beachball and all, and the log recorded it only
as silence. We decided that repo-authored workflow code never runs on the main
thread. It runs in a **workflow host**: a utility process of its own, one per
...
again, as the shipped examples' own `spawnSync('git', ...)` and the authoring
page's `execFileSync` schedule check were already teaching it. A worker thread
