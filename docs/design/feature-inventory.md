...
| Needs you | (n/a — one session, in front of you) | **built** — a turn that ends or errors while the user is not looking marks its session: amber pip, full-ink title, `(needs you)` in the accessible name, and a filled count on the workspace row. Tab walks the marks in rail order, crossing workspaces; landing clears one. While the window is unfocused main also badges the dock and posts one banner per finished session, sounding macOS's default notification sound for the first banner in a burst and silencing the ones close behind it |
